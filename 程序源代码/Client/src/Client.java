...
CommandListener{
	...
	private Form main,Mycmd,MyMessageBox,MyWebPage,MyMouse,MyMovie,MyWakeup,MyScreen;

	private StringItem si,cmdRet,successRet,sTips;
	private TextField sf,cmdline,title,context,url,mtime,moviename,dns,mac,per;//唤醒端口就默认为2012了!
	...
	private Command overlookCommand=new Command("开启/关闭使用监控",Command.ITEM,1);
	private Command monitorCommand=new Command("开启/关闭显示器",Command.ITEM,1);
	private Command cdCommand=new Command("开启/关闭光驱",Command.ITEM,1);
	private Command udiskCommand=new Command("U盘文件监控开启/关闭",Command.ITEM,1);
	private Command lockCommand=new Command("上锁/开锁",Command.ITEM,1);
        private Command shakeCommand=new Command("震窗/消震",Command.ITEM,1);
	...
	private Command mouseCommand=new Command("开锁/锁定鼠标",Command.ITEM,1);
	private Command mouseCommandM=new Command("开锁/锁定鼠标",Command.ITEM,1);
	...
	private Command wakeupCommand=new Command("唤醒远程主机",Command.ITEM,1);
	private Command wakeupCommandM=new Command("远程唤醒",Command.ITEM,1);

        private Command screenCommand=new Command("查看屏幕",Command.ITEM,1);
        private Command confirmCommand=new Command("确认比例",Command.ITEM,1);
	...
	public Client()
		...
		url=new TextField("网址","http://",50,TextField.ANY);
		...
		per=new TextField("图像缩放比例","",50,TextField.ANY);
		si=new StringItem("网络状态:","未连接到服务器");
		successRet=new StringItem("返回信息:","无");
		sf=new TextField("服务器IP:","127.0.0.1",80,TextField.ANY);
		sTips=new StringItem("千里眼帮助:","功能一览:1 关机 2 重启  3 注销 4 开机(不要怀疑这个功能) 5 DOS模拟 6 电影自动搜索下载 7 显示器操作 8 光驱操作 9 鼠标锁定 10 U盘监控 11 电脑使用监控 12 打开网页 13 弹出信息框 14 伪QQ铃音 15 窗口震动扰乱 16 屏幕查看");
		...
		main.addCommand(wakeupCommandM);
...
//按钮命令处理
	...
	if(c==startCommand)
		...
		//需要参数的操作
		main.addCommand(screenCommand);//查看屏幕
		...
		main.addCommand(movieCommandM);//自动下载电影
		...
		MyScreen=new Form("Screen");
		MyScreen.addCommand(confirmCommand);
		MyScreen.addCommand(backCommand);
